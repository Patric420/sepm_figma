...
`.trim();
  ...
  workspace.metrics.aiConfidenceScore = Math.max(85, Math.min(99, 88 + Math.floor(Math.random() * 10)));
  ...
  workspace.processing.insights.extractedRequirements = details.deliverables;
  workspace.processing.insights.keyStakeholders =
    details.stakeholders.length > 0 ? details.stakeholders : workspace.processing.insights.keyStakeholders;
  workspace.processing.insights.suggestedDeliverables = details.deliverables.slice(0, 4);
  workspace.processing.insights.timelineEstimates = [
    `Phase 1: Discovery - ${details.timeline}`,
